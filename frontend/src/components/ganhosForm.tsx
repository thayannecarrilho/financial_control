...
import { criarGanho } from '../services/api';
import {FaAlignLeft, FaMoneyBillWave, FaPlusCircle, FaStickyNote } from 'react-icons/fa';

interface GanhosFormProps {
    aba_id: number;
    onGanhoCriado: () => void;
}

export default function GanhosForm({ aba_id, onGanhoCriado }:GanhosFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await criarGanho(aba_id, descricao, parseFloat(valor), observacao);
            setDescricao('');
            setValor('');
            setObservacao('');
            onGanhoCriado();
